...
class TransversalNodeCreatorListenerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        Phake::when($this->nodeRepository)
            ->findInLastVersion(NodeInterface::TRANSVERSE_NODE_ID, 'fr', $this->siteId)
            ->thenReturn($this->nodeFr);
        Phake::when($this->nodeRepository)
            ->findInLastVersion(NodeInterface::TRANSVERSE_NODE_ID, 'en', $this->siteId)
    ...
    public function testWithAlreadyExistingNodes($method)
    {
        ...
        Phake::verify($this->nodeRepository)->findInLastVersion(NodeInterface::TRANSVERSE_NODE_ID, 'fr', $this->siteId);
        Phake::verify($this->nodeRepository)->findInLastVersion(NodeInterface::TRANSVERSE_NODE_ID, 'en', $this->siteId);
    ...
    {
        Phake::when($this->site)->getLanguages()->thenReturn(array('fr', 'en'));
        Phake::when($this->nodeRepository)->findInLastVersion(Phake::anyParameters())->thenReturn(null);
